...
class futureTripAnalysis:
    ...
    def __init__(self, travelTimeMatrix,productionMatrix, attractionMatrix,kij):
        self.travelTimeMatrix = travelTimeMatrix
        self.productionMatrix = productionMatrix
    ...
    def TripDistribution(self):
        HBW_frictionFactor = self.travelTimeMatrix * -0.9982 + 101.88
        HBO_frictionFactor = self.travelTimeMatrix * -0.8684 + 88.633
        NHB_frictionFactor = self.travelTimeMatrix *  -0.5818 + 59.384
        ...
        HBW_AjFijKij = HBW_frictionFactor * self.attractionMatrix[:,0] * self.kij
        HBO_AjFijKij = HBO_frictionFactor * self.attractionMatrix[:,1] * self.kij
        NHB_AjFijKij = NHB_frictionFactor * self.attractionMatrix[:,2] * self.kij
        ...
        HBW_tripMatrix = HBW_AjFijKij.transpose() * self.productionMatrix[:,0] / HBW_sumAjFijKij
        HBW_tripMatrix = HBW_tripMatrix.round().transpose()
        HBO_tripMatrix = HBO_AjFijKij.transpose() * self.productionMatrix[:,1] / HBO_sumAjFijKij
        HBO_tripMatrix = HBO_tripMatrix.round().transpose()
        NHB_tripMatrix = NHB_AjFijKij.transpose() * self.productionMatrix[:,2] / NHB_sumAjFijKij
    ...
    def ModelSplit(self, tripDistribution, Auto_travelTime, Bus_travelTime):
        ...
        HBW_tripAutoMatrix = Auto_Prob * tripDistribution[0]
        HBW_tripAutoMatrix = HBW_tripAutoMatrix.round()
        HBW_tripBusMatrix = Bus_Prob * tripDistribution[0]
        HBW_tripBusMatrix = HBW_tripBusMatrix.round()
        HBO_tripAutoMatrix = Auto_Prob * tripDistribution[1]
        HBO_tripAutoMatrix = HBO_tripAutoMatrix.round()
        HBO_tripBusMatrix = Bus_Prob * tripDistribution[1]
        HBO_tripBusMatrix = HBO_tripBusMatrix.round()
        NHB_tripAutoMatrix = Auto_Prob * tripDistribution[2]
        NHB_tripAutoMatrix = NHB_tripAutoMatrix.round()
        NHB_tripBusMatrix = Bus_Prob * tripDistribution[2]
    ...
    def TripTablecreator(self, modelSplittables, HBW_Auto_averageOcupancy, HBO_Auto_averageOcupancy, NHB_Auto_averageOcupancy, Bus_averageOcupancy, peakHourFactor):
        HBW_tripTable = modelSplittables[0] / HBW_Auto_averageOcupancy + modelSplittables[1] / Bus_averageOcupancy
        HBW_tripTable = np.floor(HBW_tripTable)
        HBO_tripTable = modelSplittables[2] / HBO_Auto_averageOcupancy + modelSplittables[3] / Bus_averageOcupancy
        HBO_tripTable = np.floor(HBO_tripTable)
        NHB_tripTable = modelSplittables[4] / NHB_Auto_averageOcupancy + modelSplittables[5] / Bus_averageOcupancy
